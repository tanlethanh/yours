...
function DataTestsProvider({ children }: DataTestsProviderProps) {
	...
	const [testsId, setTestsId] = useState<string>('aa');
	...
	const updateQuestionByIdService = async (
		questionId: string,
		userAnswer: undefined,
	) => {
		try {
			const res = await apiAxios.post(
				`/questions/${questionId}?action=update-answer`,
				{
					userAnswer,
				},
			);
	...
	const getTestByIdService = async (testId: string) => {
		try {
			const res = await apiAxios.get(
				`tests/${testId}?with-questions=true`,
			);
		...
		if (result?.data?.test) {
			...
			result.data.test.questions.forEach((item: any) => {
				if (item.user_answer !== undefined) {
					...
					switch (item.type) {
						case 'MULTICHOICE':
							if (item.user_answer === item.solution_index)
								countCorrect += 1;
							break;
						case 'FILLWORD':
							if (
								item.user_answer ===
								item.list_words[item.solution_index]
							)
								countCorrect += 1;
							break;

						case 'TRANSLATE':
							if (item.user_answer === item.solution)
								countCorrect += 1;
							break;
	...
	async function updateQuestionById(index: number, userAnwser: any) {
		const questionId = testsDatas[index]._id as string;
		...
		if (res) {
			...
			switch (testsDatas[index].type) {
				case 'MULTICHOICE':
					if (
						testsDatas[index].user_answer ===
						testsDatas[index].solution_index
					)
	...
	return (
		<DataTestsContext.Provider value={contextValue}>
			{children}
		</DataTestsContext.Provider>
	);
}

export { DataTestsContext, DataTestsProvider };
